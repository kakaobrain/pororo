...
class BrainRobertaHubInterface(RobertaHubInterface):
    ...
    def predict_span(
        ...
        assert self.args.task == "span_prediction", (
            f"predict_span() only works for span prediction tasks.\n"
            f"Use predict() to obtain model outputs (e.g., logits); "
            f"use predict_output() for sentence prediction tasks.")
        assert ("span_prediction_head" in self.model.classification_heads
               ), "need pre-trained span_prediction_head to make predictions"
