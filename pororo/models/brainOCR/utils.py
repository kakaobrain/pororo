...
def get_image_list(horizontal_list: list,
    ...
    for box in free_list:
        ...
        image_list.append(
            (box, crop_img))  # box = [[x1,y1],[x2,y2],[x3,y3],[x4,y4]]
    ...
    for box in horizontal_list:
        ...
        image_list.append(([[x_min, y_min], [x_max, y_min], [x_max, y_max],
                            [x_min, y_max]], crop_img))
...
def get_paragraph(raw_result,
    ...
    while len([box for box in box_group if box[7] == 0]) > 0:
        ...
        if len([box for box in box_group if box[7] == current_group]) == 0:
            box_group0[0][
                7] = current_group  # assign first box to form new group
        # try to add group
        else:
            current_box_group = [
            ...
            for box in box_group0:
                same_horizontal_level = (min_gx <= box[1] <=
                                         max_gx) or (min_gx <= box[2] <= max_gx)
                same_vertical_level = (min_gy <= box[3] <=
                                       max_gy) or (min_gy <= box[4] <= max_gy)
...
def reformat_input(image):
    ...
    if type(image) == str:
        if image.startswith("http://") or image.startswith("https://"):
            tmp, _ = urlretrieve(
                image,
                reporthook=printProgressBar(prefix="Progress:",
                                            suffix="Complete",
                                            length=50),
            )
